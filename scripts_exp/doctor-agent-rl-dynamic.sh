...
exp_name=exp-medical-consultation-1ksft-patientllm-random_t
project_name=RAGEN

python -m ragen.trainer.main_ppo \
  hydra.run.dir=outputs/exp_configs/logs/$(date +%Y-%m-%d)/$(date +%H-%M-%S) \
  data.train_files=medical_consultation/train_en_8068_all_format_filter_7k_rl_reward_w_des.parquet \
  data.val_files=medical_consultation/train_en_8068_all_format_filter_7k_rl_reward_w_des.parquet \
...
target_model=DoctorLLM-7B-8k-GRPO-Clip_Higher-1000SFTCold-DynamicTurn
