...
Our preprocessed training data is located in the `medical_consultation/` directory. For **Supervised Fine-Tuning (SFT)** cold start, we use the `train_en_8068_all_format_filter_994_sft_w_thinking.parquet` dataset. This dataset was created by prompting DeepSeek-V3 to generate the thinking process for each sample.
